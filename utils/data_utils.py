...
def get_scene_list(file, airport_code, traj_dir, max_scenes = None, sorted = True):
    ...
    if sorted: scene_file_list = order_file_tags(scene_file_list)
...
def pad_array(array, padding, dim = 1):
    shape = list(array.shape)
    shape[dim] =  padding
    shape = tuple(shape)
    try:
        bottom_padding = torch.zeros(shape).to(device="cuda")
    except:
       raise Exception
...
def get_full_scene_batch(batch, scene_dict, predictions):
    ...
    if N > K: 
        pred_scores = pad_array(pred_scores, padding= N-K)
        mu = pad_array(mu, padding = N - K)
        sigma = pad_array(sigma, padding= N -K)
